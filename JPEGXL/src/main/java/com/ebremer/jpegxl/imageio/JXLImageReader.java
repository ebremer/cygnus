...
public final class JXLImageReader extends ImageReader {
    ...
    private com.ebremer.jpegxl.io.CodestreamSource source;
    ...
    private com.ebremer.jpegxl.io.CodestreamSource source() throws IOException {
        if (source == null) {
            if (!(input instanceof ImageInputStream stream)) {
                throw new IllegalStateException("no input set");
            }
            source = new com.ebremer.jpegxl.io.CodestreamSource.StreamSource(
                    stream, com.ebremer.jpegxl.container.Container.scanSegments(stream));
        }
        return source;
    }
