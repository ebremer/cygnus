...
import com.ebremer.jpegxl.codestream.IccStream;
import com.ebremer.jpegxl.codestream.ImageMetadata;
import com.ebremer.jpegxl.codestream.SizeHeader;
import com.ebremer.jpegxl.codestream.Toc;
import com.ebremer.jpegxl.color.Transfer;
import com.ebremer.jpegxl.color.XybConverter;
import com.ebremer.jpegxl.container.Container;
import com.ebremer.jpegxl.entropy.EntropyDecoder;
import com.ebremer.jpegxl.features.Noise;
import com.ebremer.jpegxl.features.PatchesDictionary;
import com.ebremer.jpegxl.features.Splines;
...
public final class JxlDecoder {
    ...
    private static FrameResult decodeFrame(CodestreamSource src, FrameHeader fh, Toc toc,
        ...
        if (!fh.isModular) {
            state.vardct = new com.ebremer.jpegxl.vardct.VarDctState(fh, meta);
        }
        ...
        java.util.function.BiConsumer<com.ebremer.jpegxl.vardct.VarDctState, FrameHeader> cap =
                JPEG_CAPTURE.get();
    ...
    public static final ThreadLocal<java.util.function.BiConsumer<
            com.ebremer.jpegxl.vardct.VarDctState, FrameHeader>> JPEG_CAPTURE =
            new ThreadLocal<>();
    ...
    private static FrameResult buildResult(FrameState state, GroupSelection sel)
            ...
            if (fh.isSubsampled) {
                for (int c = 0; c < 3; c++) {
                    colorF[c] = upsampleChroma(colorF[c], filterW >> fh.jpegShiftX[c],
                            ...
                            com.ebremer.jpegxl.io.Bits.ceilDiv(w, 1 << fh.jpegShiftX[c]),
                            com.ebremer.jpegxl.io.Bits.ceilDiv(h, 1 << fh.jpegShiftY[c]),
        ...
        if (numEC > 0) {
            ...
            for (int i = 0; i < numEC; i++) {
                ...
                int ecW = com.ebremer.jpegxl.io.Bits.ceilDiv(fh.displayWidth, fh.ecUpsampling[i]);
                int ecH = com.ebremer.jpegxl.io.Bits.ceilDiv(fh.displayHeight, fh.ecUpsampling[i]);
    ...
    private static void upsampleFrame(FrameResult r, ImageMetadata meta) {
        ...
        for (int c = 0; c < r.planes.length; c++) {
            ...
            int pw = c < r.outColour
                    ? r.width
                    : com.ebremer.jpegxl.io.Bits.ceilDiv(fh.displayWidth, factor);
            int ph = c < r.outColour
                    ? r.height
                    : com.ebremer.jpegxl.io.Bits.ceilDiv(fh.displayHeight, factor);
    ...
    private static boolean hasSpotColours(ImageMetadata meta) {
        for (var ec : meta.extraChannels) {
            if (ec.type == com.ebremer.jpegxl.codestream.ExtraChannelInfo.TYPE_SPOT_COLOUR) {
                return true;
            }
        }
        return false;
    ...
    private static final class FrameState {
        ...
        com.ebremer.jpegxl.vardct.VarDctState vardct;
    ...
    private static void decodeLfGlobal(Bits in, FrameState state) throws IOException {
        ...
        if (fh.hasFlag(FrameHeader.FLAG_PATCHES)) {
            int numAlpha = 0;
            for (var ec : meta.extraChannels) {
                if (ec.type == com.ebremer.jpegxl.codestream.ExtraChannelInfo.TYPE_ALPHA) {
                    numAlpha++;
                }
            }
