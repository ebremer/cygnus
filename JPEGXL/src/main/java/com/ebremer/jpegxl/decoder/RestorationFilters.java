...
final class RestorationFilters {
            ...
            JxlDecoder.parallelFor(height, y -> {
                ...
                for (int x = 0; x < width; x++) {
                    ...
                    for (int[] cross : crossList) {
                        float dist = iterF == 2
                                ? distance2(src, colors, rf, y, x, cross, width, height)
                                : distance1(src, colors, rf, y, x, cross, width, height);
                        float weight = weight(rf, sigmaScale, dist, invSigma, y, x);
                        sumWeights += weight;
                        int my = mirror(y + cross[0], height);
                        int mx = mirror(x + cross[1], width);
                        for (int c = 0; c < colors; c++) {
    ...
    private static float distance1(float[][] buf, int colors, RestorationFilter rf,
            int by, int bx, int[] dCross, int width, int height) {
        float dist = 0f;
        for (int c = 0; c < 3; c++) {
            float[] p = buf[colors == 1 ? 0 : c];
            float scale = rf.epfChannelScale[c];
            for (int[] cross : CROSS) {
                int py = mirror(by + cross[0], height);
                int px = mirror(bx + cross[1], width);
                int dy = mirror(by + dCross[0] + cross[0], height);
                int dx = mirror(bx + dCross[1] + cross[1], width);
    ...
    private static float distance2(float[][] buf, int colors, RestorationFilter rf,
            int by, int bx, int[] cross, int width, int height) {
        float dist = 0f;
        for (int c = 0; c < 3; c++) {
            float[] p = buf[colors == 1 ? 0 : c];
            int dy = mirror(by + cross[0], height);
            int dx = mirror(bx + cross[1], width);
